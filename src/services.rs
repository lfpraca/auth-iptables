use actix_web::{get, web::{self, Data}, HttpResponse, HttpRequest, Responder};
...
async fn update_ip(state: Data<AppState>, path: web::Path<String>, req: HttpRequest) -> impl Responder {
    ...
    .args(&[
        ...
        "-p", "tcp", "-m", "state",
        "--state", "NEW", "-m", "tcp",
        "--dport", &state.dest_port.to_string(),
        "-s", &old_ip, "-j", "ACCEPT"
    ])
    .stdout(Stdio::null())
    .stderr(Stdio::null())
    ...
    if state.final_reject {
        {
            ...
            if !delete_output.status.success() {
                return HttpResponse::InternalServerError().body("Error deleting final reject input rule\n");
            }
        }
        {
            ...
            if !create_output.status.success() {
                return HttpResponse::InternalServerError().body("Error creating final reject input rule\n");
            }
        }
    }
